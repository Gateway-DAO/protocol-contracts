...
contract DataModel {
    ...
    function createModelVersion(
        ...
        modelVersions[
            keccak256(abi.encodePacked(_id, _version))
        ] = ModelVersion(_version, _uri);
        models[_id].versions.push(_version);
    }

    function getModelVersion(string memory _id, string memory _version)
        public
        view
    ...
    function getModel(string memory _id) public view returns (Model memory) {
        return models[_id];
    }
