...
contract OrgID {
    ...
    modifier isOwner() {
        require(
            members[msg.sender] && msg.sender == owner,
            "OrgID: Not owner"
        );
        _;
    }
    ...
    constructor(address _owner, address[] memory _signers) {
        require(_owner != address(0x0) || _owner != address(this), "OrgID: Invalid owner address");
        
        for (uint256 i = 0; i < _signers.length; i++) {
            require(_signers[i] != address(0x0) || _signers[i] != address(this), "OrgID: Invalid signer address");
     ...
     */
    function addMember(address _wallet)
        public
        isOwner
    {
    ...
    function transferOwnership(address _newOwner) public isOwner {
        require(_newOwner != address(0), "OrgID: New owner address cannot be 0x0");
