 ...
 */
     ...
     */
    modifier isMaster {
        require(msg.sender == wallets[master_wallet_idx].wallet && wallets[master_wallet_idx].is_master == true, "GatewayID: Not master wallet");
        _;
    }
     ...
     */
    modifier isSigner {
        require(msg.sender == wallets[signer_wallet_idx].wallet || msg.sender == wallets[master_wallet_idx].wallet, "GatewayID: Not signer wallet");
        _;
    }
    ...
    modifier isMember(bytes32 _pkh) {
        require(msg.sender == wallets[signer_wallet_idx].wallet || msg.sender == wallets[master_wallet_idx].wallet || wallets[abi.encodePacked(bytes32(_pkh), msg.sender)].wallet != address(0x0), "GatewayID: Not a member");
        _;
    }
    ...
    constructor(address _master, address _signer) {
        ...
        wallets[master_wallet_idx] = Wallet(wallet_indices.length, master, 0x0, 0x0, Type.EVM, true, false);

        if (_signer != address(0x0)) {
            signer_wallet_idx = abi.encodePacked(bytes32(0x0), _signer);
            wallet_indices.push(signer_wallet_idx);
            wallets[signer_wallet_idx] = Wallet(wallet_indices.length, _signer, 0x0, 0x0, Type.EVM, false, true);
        }
    ...
    function addEVMWallet(
        ...
        require(wallets[abi.encodePacked(address(0x0), msg.sender)].wallet == address(0x0), "GatewayID: Wallet already exists");

        wallet_indices.push(abi.encodePacked(address(0x0), msg.sender));
        wallets[abi.encodePacked(address(0x0), msg.sender)] = Wallet(wallet_indices.length - 1, _wallet, 0x0, 0x0, Type.EVM, false, false);
    ...
    function removeEVMWallet(address _member) public isMember(bytes32(0x0)) {
        ...
        require(wallets[id].wallet_type == Type.EVM, "GatewayID: Not an EVM wallet");
        require(wallets[id].is_master == false, "GatewayID: Cannot remove master wallet");

        bytes memory keyToMove = wallet_indices[wallet_indices.length - 1];
        uint rowToReplace = wallets[id].idx;
     ...
     */
    function changeMaster(address _member) public isMaster returns (bool _success) {
        require(wallets[abi.encodePacked(bytes32(0x0), _member)].wallet != address(0x0), "GatewayID: Not a member");
        require(wallets[abi.encodePacked(bytes32(0x0), _member)].wallet_type == Type.EVM, "GatewayID: Not an EVM wallet");
